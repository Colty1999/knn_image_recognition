...
def init_dataset(opt, mode):  # CHANGED
    transform = transforms.Compose([
        ...
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # Normalize to pretrained ResNet50 stats
    ])

    dataset = datasets.ImageFolder(root='reduced_dataset', transform=transform)
...
def init_dataloader(opt, mode):
    ...
    # Using dataset.targets to get the labels
...
def train(opt, tr_dataloader, model, optim, lr_scheduler, val_dataloader=None):
    ...
    for epoch in range(opt.epochs):
        ...
        postfix = ' (Best)' if avg_val_acc >= best_acc else ' (Best: {})'.format(best_acc)
    ...
    plot_learning_curves(train_loss, train_acc)

    # Optionally, if you want to plot validation loss/accuracy
    if val_dataloader:
        plot_learning_curves(val_loss, val_acc)
...
def eval(opt):
    ...
    options.dataset_root = os.getenv('DATA_DIR')  # Use the environment variable
